...
Surprisingly, $f1(5) = f1(6) = f1(7) = 13$. So we just need something that results in 5, 6 or 7 and needs the time. What about `int(time.time()) % 3 + 5`? YEEEP, that's is. So stupid simple.

So `13` becomes (with `t` as the current unix time as int)

$$ f(b,t) = -e^{i\pi} + i^{1234567 >> -24*(t \mod 3+5)^3 + 432*(t \mod 3)^2 - 2568*(t \mod 3) + 5053 >> \left(-e^{i\pi} - e^{i\pi} - e^{i\pi}\right)!} \cdot b $$
```
